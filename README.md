...
- **1 ページ内で目的ごとに整理**: `何を見たい？` で `ランキングだけ見る` / `今の装備から次を探す` / `保存した候補の比較` / `今の時給` / `魚ごとの詳細` を切り替えます。
- **上から順に使える**: まず `1. 条件を入れる` と `2. 見たいものを選ぶ` を済ませ、ランキング/候補探しでは `3. 対象を決める` → `4. ランキングや候補を見る` の順に読めるようにしています。
...
- **ランキングだけ見たい人も迷わない**: デフォルトは `ランキングだけ見る` で、比較に追加する前にまず順位だけを眺められます。
...
5. **`何を見たい？` で目的を選ぶ**: まずは `ランキングだけ見る` で全体順位を眺めるか、`今の装備から次を探す` で候補追加まで進めるかを決めます。

6. **ランキングだけ見たいときは、そのまま順位を見る**: `3. ランキング対象を決める` で `Rod / Line / Bobber / Enchant` を1つ選ぶと個別ランキング、複数選ぶと組み合わせランキングになります。

7. **`4. ランキングを見る` で順位を追う**: 先に `この条件の目安` を見てから、同じカードの下で上位帯・下位帯・特定順位帯をそのまま追えます。
