...
- **Step 1 は左の現在装備表を起点に進む**: Rod / Line / Bobber / Enchant の4行を見ながら、変えたい行を押すと右にその欄の候補一覧が出ます。
- **Step 1 はゲーム画面っぽい loadout board で選ぶ**: 現在の装備は暗い HUD 風のボードにまとまっており、編集中の欄は色と発光で追えます。右の候補一覧はインベントリの引き出しのように見えるよう整理しています。
...
1. **左の「今の装備の表」から現在の装備をそろえる**: Rod / Line / Bobber / Enchant の4行を見て、変えたい行を押します。右に出る候補一覧で選ぶと、その行にそのまま反映されます。Fishing Area は最初は自動のままで大丈夫です。
