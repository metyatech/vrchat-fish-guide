...
- **先に目的を選ぶ**: `Rod / Line / Bobber / Enchant / 全部まとめて` から比べたい対象を選ぶと、その目的に合わせた次の候補を最上部に表示します。
- **次の候補を先に出す**: 現在の装備と比較し、次に試すべき候補と期待値/時間の差を先に提示します。
...
1. **場所と現在の装備を入れる**: `確率計算機` で Fishing Area と現在の Rod / Line / Bobber / Enchant を選びます。

2. **比べたい枠を 1 つ選ぶ**: 上部の `Rod / Line / Bobber / Enchant / 全部まとめて` から、いま比較したい対象を選びます。

3. **次に試す候補を見る**: 画面上部の `次にやること` カードに、その条件で次に試すべき候補が出ます。

4. **比較候補を追加する**: `この候補で比べる組み合わせを追加` を押すと、現在の装備を基準に 1 か所だけ変えた組み合わせが追加されます。

5. **期待値/時間で判断する**: 比較テーブルで `期待値/時間` を先に見て、必要なら `期待値/回` と `魚が釣れる確率` を確認します。

6. **さらに詰める**: `次に試す候補` で単一枠の候補を見たり、`全部まとめて比べる` で全組み合わせ上位候補を見たりできます。

7. **必要なときだけ詳細調整**: `詳細調整` を開くと、推定前提、見た目・サイズ補正、レア度上書きを調整できます。
