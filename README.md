...
- 1 本の自然な操作フローで比較を進める guided UX
- 「何を比べたいか」を先に選び、次に試す候補を即表示
...
- **比較ビルド追加**: 現在の装備を基準に、1 枠だけ変えた比較ビルドを増やして差分を見られます。
- **ビルド操作**: ダブルクリック・✏ で名前変更、⧉ で複製、✕ で削除。
- **URL シェア**: 「🔗 URL をコピー」で現在のビルド構成（複数ビルド含む）を URL に保存。この URL を友人と共有したり、後で復元できます。

### Guided comparison flow

- **Single-flow UX**: beginner / expert のモード分岐は置かず、1 つの画面の中で必要な順に入力と結果を並べています。
- **Compare target chooser**: `Rod / Line / Bobber / Enchant / Full build` を先に選ぶと、その目的に合わせた次の候補を最上部に表示します。
- **Next recommendation card**: 現在の装備と比較し、次に試すべき候補と期待値/時間の差を先に提示します。
- **Advanced details stay in the same path**: `詳細調整` に experimental な前提や rarity override をまとめ、必要になるまで初期表示から退かせています。
...
1. **Area と現在の装備を入れる**: `確率計算機` で Fishing Area と現在の Rod / Line / Bobber / Enchant を選びます。

2. **比べたい枠を 1 つ選ぶ**: 上部の `Rod / Line / Bobber / Enchant / Full build` から、いま比較したい対象を選びます。

3. **次に試す候補を見る**: 画面上部の `次にやること` カードに、その条件で次に試すべき候補が出ます。

4. **比較ビルドを作る**: `この候補で比較ビルドを作る` を押すと、現在の装備を基準に 1 枠だけ変えた比較ビルドが追加されます。

5. **期待値/時間で判断する**: `ビルド比較` テーブルで `期待値/時間` を先に見て、必要なら `期待値/回` と `魚が釣れる確率` を確認します。

6. **さらに詰める**: `スロット別ランキング` で単一枠の候補を見たり、`フルビルド最適化` で全組み合わせ上位候補を見たりできます。

7. **必要なときだけ詳細調整**: `詳細調整` を開くと、experimental 前提、modifier assumptions、rarity override を調整できます。
