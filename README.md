...
- フルビルド最適化（理論上 41,280 通りの候補から、完全劣後の装備を除いた exact search で EV/時間 上位ビルドを提示）
...
- **上から順に使える**: `今の装備` → `必要なら場所と条件` → `プレイ速度の微調整` → `1 つ変える欄を選ぶ` → `候補を 1 件追加` → `比較結果` の順に並べています。
...
- `期待値重視` と `バランス型` のタグが増えたので、「とにかく強い候補」と「複数ステータスが素直に伸びる候補」を切り分けて見られます。
...
- 「全部まとめて比べる」を開くと、全装備の組み合わせ候補（理論上 15 × 8 × 8 × 43 = **41,280 通り**）から、能力的に完全に劣る Rod / Line / Bobber を先に外した exact search を行い、期待値/時間の上位候補を一覧表示します。
...
- 初期表示は上位 10 件。「もっと見る」で上位 100 件まで表示を拡張できます（再検索不要）。
...
4. **次に、1 つだけ変えて試す欄を選ぶ**: `Rod / Line / Bobber / Enchant / 全部まとめて入れ替える` から、いま見たい比較方法を選びます。

5. **候補を 1 件追加する**: `まずはこの候補を比較へ追加` または候補一覧の `この候補を追加` を押します。

6. **保存した候補を並べて比べる**: 比較欄で、いま追加した候補と現在の装備を並べます。

7. **期待値/時間で判断する**: まず `期待値/時間` を見て、必要なら `期待値/回` と `魚が釣れる確率` を確認します。

8. **最後に詳しい数字を見る**: 下のグラフや魚一覧で、どの魚が期待値を押し上げているかを確認します。

9. **必要なときだけ詳細調整**: `詳細調整` を開くと、推定前提、見た目・サイズ補正、レア度上書きを調整できます。
