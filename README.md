...
- **上から順に使える**: `1. 何をしたいか決める` → `2. 条件と基準装備を決める` を済ませ、ランキング/候補探しでは `3. 対象を決める` → `4. ランキングや候補を見る` の順に読めるようにしています。
...
1. **最初に `何を見たい？` を選ぶ**: まず `ランキングだけ見る` / `今の装備から次を探す` / `保存した候補の比較` / `今の時給` / `魚ごとの詳細` のどれをしたいか決めます。
